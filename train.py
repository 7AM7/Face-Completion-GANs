...
def build_model():
    ...
    for n in range(n_epoch):
        ...
        for i in range(int(len(X_train) // batch_size)):
            ...
            masked = inputs * (1 - masks)
            completion_image = generator.predict([masked, masks])
            g_loss = 0.0
            d_loss = 0.0
            if n < tc:
                g_loss = generator.train_on_batch([masked, masks], inputs)
            else:
                ...
                if n >= tc + td:
                    g_loss = all_model.train_on_batch([masked, masks, points],
                                                      [inputs, valid])
